...
<html lang="du">
  <head>
    ...
    <nav class="navbar navbar-inverse navbar-fixed-top">
      <div class="container">
        <div align="left">
		<a class="navbar-brand" href="##" style="color:#BEF781;text-align:" >Dagboek</a>
        </div>
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
			//$rndSfeer = (round($row[sfeer,2]);
			$sfeer = substr($row[cafe], 4);    // returns "f"
			$assortiment = substr($row[sfeer], 4);    // returns "f"
			$muziek = substr($row[muziek], 4);    // returns "f"
			$publiek = substr($row[publiek], 4);    // returns "f"
			$behang = substr($row[behang], 4);    // returns "f"

			echo "<div class=\"row\" style=\"background-color:lavenderblush;\">
					<div class=\"col-xs-1 col-sm-1 col-md-1 col-lg-1\">
					  <p>substr ( string $row[userid] , 0,4 )</p>
					</div>
					<div class=\"col-xs-2 col-sm-2 col-md-2 col-lg-2\">
					  <p></p>
					</div>
					<div class=\"col-xs-2 col-sm-2 col-md-2 col-lg-2\">
					  <p>substr ( string $row[sfeer] , 0,4 )</p>
					</div>
					<div class=\"col-xs-2 col-sm-2 col-md-2 col-lg-2\">
					  <p>$row[muziek]</p>			
					</div>
